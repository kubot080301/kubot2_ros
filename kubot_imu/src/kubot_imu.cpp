...
KubotIMU::KubotIMU(ros::NodeHandle nh, ros::NodeHandle pnh):
  ...
  is_calibrated_(false)
{
  ...
  raw_sub_ = nh_.subscribe("raw_imu", 5, &KubotIMU::rawCallback, this);

  imu_cal_srv_ = nh_.advertiseService("imu/calibrate_imu", &KubotIMU::calibrateCallback, this);
  ...
  if (use_accelerometer_ || use_gyroscope_)
  {
    ...
    KubotIMU::fillRowMajor(linear_acc_covar_, linear_acc_stdev_);

    pnh_.param<double>("imu/angular_vel_stdev", angular_vel_stdev_, 0.0);
    KubotIMU::fillRowMajor(angular_vel_covar_, angular_vel_stdev_);
  }
  ...
  if (use_magnetometer_)
  {
    ...
    if (use_mag_msg_)
    {
      ...
      KubotIMU::fillRowMajor(magnetic_field_covar_, magnetic_field_stdev_);
    }
    else
    {
...
void KubotIMU::rawCallback(const kubot_msgs::RawImuConstPtr& raw_msg)
{
...
bool KubotIMU::calibrateCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
...
void KubotIMU::fillRowMajor(boost::array<double, 9> & covar, double stdev)
{
